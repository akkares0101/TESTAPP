...
function SocialImportantDaysPage({ isMuted, onVideoStateChange }) {
  ...
  // 📅 Path วิดีโอหลัก
  const mainVideo = "/videos/social/importantdays.mp4";
  ...
  return (
    ...
    >
      ...
      {/* 1. ส่วนหัว (❌ ไม่มีปุ่มกลับ โทนสีเหลือง Yellow) */}
      <div className="w-full px-4 flex justify-center items-center z-20 shrink-0 mb-2 md:mb-4">
        <div className="px-8 py-2 md:px-12 md:py-3 rounded-[3rem] shadow-lg border-[4px] md:border-[6px] border-yellow-300 bg-white/90 backdrop-blur-md">
           <h1 className="text-xl md:text-3xl lg:text-4xl font-black tracking-wide text-yellow-600 text-center drop-shadow-sm">
             📅 เรื่อง วันสำคัญ
           </h1>
        </div>
      </div>
      ...
      <div className="w-full flex-1 flex flex-col items-center justify-center z-10 px-4 min-h-0">
        <div className="relative w-full max-w-[950px] aspect-video max-h-[70vh] bg-black rounded-[2rem] border-[6px] md:border-[8px] border-yellow-400 shadow-[0_10px_0_#facc15] overflow-hidden group mb-4 md:mb-6">
            <video
                ref={videoRef}
                src={mainVideo}
