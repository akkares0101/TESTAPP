...
function FamilyGamePage({ isMuted }) {
  ...
  const [items] = useState([
    { id: 'father', word: 'Father', thai: 'คุณพ่อ', image: imgFather, color: '#3B82F6' },   // เส้นสีฟ้า
    { id: 'mother', word: 'Mother', thai: 'คุณแม่', image: imgMother, color: '#EC4899' },   // เส้นสีชมพู
    { id: 'sister', word: 'Sister', thai: 'พี่สาว/น้องสาว', image: imgSister, color: '#F59E0B' }, // เส้นสีส้ม
    { id: 'grandpa', word: 'Grandfather', thai: 'ปู่ / ตา', image: imgGrandpa, color: '#10B981' }, // เส้นสีเขียว
    { id: 'grandma', word: 'Grandmother', thai: 'ย่า / ยาย', image: imgGrandma, color: '#8B5CF6' }, // เส้นสีม่วง
  ]);
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-5xl px-4 flex-1 flex items-center justify-center" ref={containerRef}>
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" style={{ overflow: 'visible', filter: 'drop-shadow(0px 2px 4px rgba(0,0,0,0.2))' }}>
          {lines.map((line, i) => (
             <g key={i}>
                {/* เส้นขอบสีขาว */}
               <line x1={line.startX} y1={line.startY} x2={line.endX} y2={line.endY} stroke="white" strokeWidth="12" strokeLinecap="round" />
                {/* เส้นสีจริง */}
               <line x1={line.startX} y1={line.startY} x2={line.endX} y2={line.endY} stroke={line.color} strokeWidth="8" strokeLinecap="round" />
             </g>
          ))}
          {currentLine && (
             <line x1={currentLine.startX} y1={currentLine.startY} x2={currentLine.endX} y2={currentLine.endY} stroke={currentLine.color} strokeWidth="8" strokeLinecap="round" strokeDasharray="15,10" className="opacity-80" />
          )}
        </svg>
               ...
               return (
                <div key={item.id} className="relative flex items-center">
                  ...
                  <div className={`
                      ...
                      ${isMatched ? 'border-green-400 grayscale opacity-70' : 'border-white hover:scale-110'}
                  `}>
                     <img src={item.image} alt={item.word} className="w-full h-full object-contain rounded-full" />
                  </div>
                ...
                <div key={item.id} className="relative flex items-center justify-end">
                  ...
                  <div 
                    ...
                    className={`
                      ...
                      ${isMatched ? 'bg-green-400 scale-95 opacity-80' : 'hover:bg-yellow-300 hover:scale-105'}
                    `}
                  >
                    <h2 className="text-xl md:text-2xl font-black text-gray-800">{item.word}</h2>
                    <p className="text-sm md:text-lg text-white font-bold drop-shadow-sm">{item.thai}</p>
                  </div>
      ...
      {score === items.length && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
           <div className="bg-white rounded-[3rem] p-8 md:p-12 max-w-lg w-full text-center border-[8px] border-yellow-400 shadow-2xl animate-bounce-in">
              <h2 className="text-5xl font-black text-orange-500 mb-4">Well Done! 🎉</h2>
              <div className="flex justify-center gap-2 mb-6 text-6xl">
                 ⭐ ⭐ ⭐
              </div>
              <p className="text-2xl text-gray-600 font-bold mb-8">จับคู่ครบทุกข้อแล้ว!</p>
              <div className="flex flex-col gap-3">
                 <button onClick={resetGame} className="bg-green-500 text-white text-xl font-black py-3 rounded-full shadow-lg hover:scale-105">🔄 เล่นอีกครั้ง</button>
                 <button onClick={() => navigate('/family')} className="bg-white text-orange-500 text-xl font-black py-3 rounded-full border-4 border-orange-200">กลับเมนูหลัก</button>
              </div>
           </div>
        </div>
      )}
