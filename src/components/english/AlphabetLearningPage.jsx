...
function AlphabetLearningPage({ isMuted }) {
  ...
  return (
    ...
    >
      ...
      {/* 2. Video Player - ✅ จอใหญ่ (65% ของความสูงหน้าจอ) */}
      <div className="w-full max-w-4xl px-4 flex-1 min-h-0 flex flex-col items-center justify-center z-10 mb-2">
        <div className="w-full h-full max-h-[65vh] bg-black rounded-[2rem] border-[6px] border-rose-200 shadow-[0_15px_40px_rgba(0,0,0,0.4)] relative overflow-hidden flex flex-col">
          <video 
            ref={videoRef}
      ...
      {/* 3. Buttons Area - ✅ ขนาดปุ่มกลับมาปกติ แต่ใช้พื้นที่ 35vh */}
      <div className="w-full max-w-4xl h-[35vh] shrink-0 bg-white/60 backdrop-blur-xl rounded-t-[2rem] border-t-4 border-white/50 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] flex flex-col z-10">
        ...
        <div className="flex flex-col gap-2 py-2 shrink-0 border-b border-white/30 px-4">
            <div className="flex justify-center gap-3">
                <button 
                    ...
                    className={`px-4 py-1.5 md:px-5 md:py-2 rounded-full font-bold text-sm md:text-base transition-all shadow-sm border-2 ${isUpperCase && viewMode === 'standard' ? 'bg-blue-500 text-white border-blue-600' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
                >
                    ABC พิมพ์ใหญ่
                </button>
                <button 
                    onClick={() => { setIsUpperCase(false); setViewMode('standard'); }}
                    className={`px-4 py-1.5 md:px-5 md:py-2 rounded-full font-bold text-sm md:text-base transition-all shadow-sm border-2 ${!isUpperCase && viewMode === 'standard' ? 'bg-pink-500 text-white border-pink-600' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
                >
                    abc พิมพ์เล็ก
                </button>
            </div>
            ...
            <div className="flex justify-center gap-2">
                <button 
                    ...
                    className={`px-3 py-1.5 rounded-full font-bold text-xs md:text-sm transition-all shadow-sm border-2 flex items-center gap-1.5 ${viewMode === 'writing_upper' ? 'bg-orange-500 text-white border-orange-600' : 'bg-white text-orange-500 border-orange-200'}`}
                >
                    วิธีเขียน (ใหญ่)
                </button>
                <button 
                    onClick={() => setViewMode('writing_lower')}
                    className={`px-3 py-1.5 rounded-full font-bold text-xs md:text-sm transition-all shadow-sm border-2 flex items-center gap-1.5 ${viewMode === 'writing_lower' ? 'bg-purple-500 text-white border-purple-600' : 'bg-white text-purple-500 border-purple-200'}`}
                >
                    วิธีเขียน (เล็ก)
                </button>
            </div>
        ...
        {/* ตารางตัวอักษร - ✅ ปุ่มขนาดเดิม w-10 h-10 */}
