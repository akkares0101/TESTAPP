      ...
      <div className="flex-1 flex flex-col justify-center items-center w-full max-w-6xl px-4 z-10">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-[3rem] px-10 py-6 md:px-20 md:py-8 shadow-[0_10px_40px_rgba(0,0,0,0.1)] mb-10 border-4 border-white relative group">
           ...
           <h2 className="text-3xl md:text-5xl font-black text-gray-700 text-center">
             Odd One Out: <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-pink-500">{currentQuestion.title}</span>
           </h2>
           <p className="text-center text-gray-400 mt-2 font-bold text-lg">({currentQuestion.desc})</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-12 w-full">
          {currentQuestion.options.map((option, index) => {
            ...
            if (isAnswered) {
                ...
                cardStyle = "bg-white/80 border-white hover:border-purple-300 hover:bg-white";
                effect = "hover:scale-105 hover:shadow-2xl hover:-translate-y-2 animate-float";
            }

            return (
              <div key={option.id} className="relative group" style={{ animationDelay: `${index * 0.2}s` }}>
                ...
                <button
                  ...
                  `}
                >
                  <img src={option.image} alt={option.word} className="w-24 h-24 md:w-40 md:h-40 object-contain drop-shadow-md" />
                  <span className="text-xl md:text-3xl font-black text-gray-700">{option.word}</span>
                </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
           <div className="bg-white rounded-[3rem] p-8 md:p-12 max-w-lg w-full text-center border-[10px] border-yellow-300 shadow-[0_20px_60px_rgba(0,0,0,0.5)] relative overflow-hidden animate-bounce-in">
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-3xl p-6 mb-8 border-4 border-purple-100 shadow-inner">
                ...
                <p className="text-7xl font-black text-green-500 mt-2 drop-shadow-sm">{score} <span className="text-3xl text-gray-400">/{questions.length}</span></p>
              </div>
              ...
              <div className="flex flex-col gap-3">
                 ...
                 <button onClick={() => navigate('/alphabet')} className="w-full bg-white text-gray-500 text-xl font-black py-4 rounded-full border-4 border-gray-200 hover:bg-gray-50 hover:border-gray-300 transition-all">
                   Back to Menu
                 </button>
              </div>
           </div>
