...
function PhonicsVowelsPage({ isMuted, onVideoStateChange }) {
  ...
  // ▶️ สั่งให้วิดีโอเล่นทันที + ปรับระดับเสียง
  useEffect(() => {
    if (videoRef.current) {
  ...
  const timeStamps = [
    { id: 1, char: "A", time: 64, color: "bg-red-500", border: "border-red-600" },    
    ...
    { id: 5, char: "U", time: 155, color: "bg-purple-500", border: "border-purple-600" }, 
  ];
  ...
  return (
    ...
    >
      ...
      {/* 1. Header (❌ ไม่มีปุ่มกลับ จัดกึ่งกลาง) */}
      ...
      <div className="w-full max-w-[950px] px-2 md:px-4 flex-1 min-h-0 flex flex-col justify-center pb-2 z-10">
        <div className="relative w-full aspect-video max-h-[55vh] md:max-h-[60vh] bg-black rounded-[2rem] border-[6px] md:border-[8px] border-yellow-400 shadow-[0_10px_0_#d97706] overflow-hidden group">
            <video
                ref={videoRef}
                src={mainVideo}
      ...
      <div className="w-full max-w-[950px] shrink-0 bg-white/80 backdrop-blur-xl rounded-t-[2.5rem] border-t-4 border-white shadow-[0_-10px_40px_rgba(0,0,0,0.1)] flex flex-col items-center z-20 pt-4 pb-6">
        ...
        <div className="flex flex-col items-center w-full px-6">
            <span className="text-blue-500 font-bold text-sm mb-3 bg-blue-50 px-4 py-1 rounded-full border border-blue-100">
                เลือกสระเพื่อฟังเสียงเพลงได้เลยจ้า 🎵
            </span>

            {/* ปุ่มทางลัด A E I O U (ทรงสี่เหลี่ยมโค้งมนสไตล์แอปเด็ก) */}
