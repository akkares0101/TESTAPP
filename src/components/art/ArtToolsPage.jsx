...
function ArtToolsPage({ isMuted, onVideoStateChange }) {
  ...
  return (
    ...
    >
      ...
      {/* 1. ส่วนหัว (ลด margin ลงนิดนึง) */}
      <div className="w-full px-4 flex justify-center items-center z-20 shrink-0 mb-2 md:mb-4">
        <div className="px-8 py-2 md:px-12 md:py-3 rounded-[3rem] shadow-lg border-[4px] md:border-[6px] border-indigo-300 bg-white/90 backdrop-blur-md">
           <h1 className="text-xl md:text-3xl lg:text-4xl font-black tracking-wide text-indigo-600 text-center drop-shadow-sm">
             🖌️ เรื่อง อุปกรณ์สร้างงานศิลปะ
           </h1>
        </div>
      </div>

      {/* 2. Video Player Area (ปรับ max-h ให้กระชับขึ้นเพื่อกันสกอร์บาร์) */}
      <div className="w-full flex-1 flex flex-col items-center justify-center z-10 px-4 min-h-0">
        <div className="relative w-full max-w-[950px] aspect-video max-h-[60vh] bg-black rounded-[2rem] border-[6px] md:border-[8px] border-indigo-400 shadow-[0_10px_0_#4f46e5] overflow-hidden group mb-4 md:mb-6">
            <video
                ref={videoRef}
                src={mainVideo}
        ...
        {/* ⭐ 3. ปุ่มกดหยุด (ปรับขนาดให้กระชับขึ้น) */}
        <button 
          onClick={togglePlay}
          className={`
